...
const CONTENT_TYPE_CONFIG: Record<string, { label: string; color: string; bgColor: string }> = {
  'original': { label: '原文', color: 'text-amber-400', bgColor: 'bg-amber-500/10' },
  'note': { label: '注', color: 'text-blue-400', bgColor: 'bg-blue-500/10' },
  'commentary': { label: '疏', color: 'text-purple-400', bgColor: 'bg-purple-500/10' },
  'translation': { label: '译文', color: 'text-green-400', bgColor: 'bg-green-500/10' },
};
    ...
    <div className="space-y-6">
      ...
      {grouped.original.map((item, idx) => (
        <div key={item.id || idx} className="space-y-2">
          ...
          <div className="text-xl leading-loose text-white/90 font-serif">
            {item.content.split('\n').map((line, i) => (
        ...
        <div className="border-t border-white/10 pt-6">
          {grouped.translation.map((item, idx) => (
            <div key={item.id || idx} className="space-y-2">
              ...
              <div className="text-lg leading-loose text-white/70">
                {item.content.split('\n').map((line, i) => (
                  <p key={i} className="mb-2">{line}</p>
                ))}
              </div>
      ...
      {grouped.note.length > 0 && (
        <div className="border-t border-white/10 pt-6">
          ...
          {grouped.note.map((item, idx) => (
            <div key={item.id || idx} className="mb-4 p-4 rounded-lg bg-blue-500/5 border border-blue-500/20">
              {item.source && (
                <p className="text-sm text-blue-400 mb-2">{item.source}</p>
              )}
      ...
      {grouped.commentary.length > 0 && (
        <div className="border-t border-white/10 pt-6">
          ...
          {grouped.commentary.map((item, idx) => (
            <div key={item.id || idx} className="mb-4 p-4 rounded-lg bg-purple-500/5 border border-purple-500/20">
              {item.source && (
                <p className="text-sm text-purple-400 mb-2">{item.source}</p>
              )}
              <div className="text-white/70 leading-relaxed">
                {item.content.split('\n').map((line, i) => (
                  <p key={i} className="mb-1">{line}</p>
