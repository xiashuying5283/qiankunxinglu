      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          <div className="flex items-center justify-center mb-4">
            ...
            <h1 className="text-4xl font-bold text-amber-500">古籍阅读</h1>
          </div>
          <p className="text-muted-foreground">研习经典，传承智慧</p>
        </div>
        ...
        <div className="max-w-5xl mx-auto">
          {Object.keys(booksByCategory).length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground text-sm mt-2">请稍后再来</p>
            </div>
          ) : (
            Object.entries(booksByCategory).map(([category, categoryBooks]) => {
              ...
              return (
                <div key={category} className="mb-10">
                  <h2 className="text-xl font-medium text-amber-400 mb-6 flex items-center gap-2">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    {categoryBooks.map((book) => (
                      <Link key={book.id} href={`/books/${book.id}`}>
                        <Card className="bg-card/50 border-amber-500/20 hover:border-amber-500/40 hover:bg-card transition-all cursor-pointer group h-full">
                          <CardHeader>
                            <div className="flex items-start justify-between">
                              <div>
                                <CardTitle className="text-2xl text-amber-500 group-hover:text-amber-400 transition-colors">
                                  {book.title}
                                </CardTitle>
                                {book.author && (
                          ...
                          <CardContent>
                            {book.description && (
                              <p className="text-muted-foreground leading-relaxed line-clamp-2">{book.description}</p>
                            )}
